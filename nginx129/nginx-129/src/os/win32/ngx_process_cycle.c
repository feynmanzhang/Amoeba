...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    for ( ;; ) {
        ...
        if (ev == WAIT_OBJECT_0 + 3) {
            ...
            cycle->dbconn = 1;///zll dbconn  of master process should be 1,means it could not connet to DB.
            cycle = ngx_init_cycle(cycle);
            cycle->dbconn = 0;///zll dbconn  of work process should be 0,means it could connet to DB.
            //ngx_log_error(NGX_LOG_ALERT, cycle->log, 0,"cycle's dbconn(\"%d\") failed",cycle->dbconn);
            if (cycle == NULL) {
