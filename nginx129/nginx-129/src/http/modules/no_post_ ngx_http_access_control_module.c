...
ngx_http_access_control_handler(ngx_http_request_t *r)
{
    ...
    if (!(r->method & (NGX_HTTP_GET|NGX_HTTP_POST))) {// only get and post.
...
//        {
//        	ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,"zll1");
//
//        	ngx_http_read_client_request_body(r,ngx_http_access_control_body_handler);
//
//        	if(r->request_body == NULL)
//        		return NGX_HTTP_BAD_REQUEST;
//
//        	ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,"zll2");
//            //catch the 'sid' parameter
//            if((sidstrstart = ngx_strcasestrn(r->request_body->buf->start,"sid=",3)) != NULL)
//            {
//            	sid.data = sidstrstart+4;
//
//            	if((sidstrend = ngx_strlchr(sidstrstart,r->request_body->buf->end,'&'))== NULL)
//            		sid.len = r->uri_end - sidstrstart - 4;
//            	else
//            		sid.len = sidstrend - sidstrstart - 4;
//
//            	 ngx_log_error(lrcf->limit_log_level, r->connection->log, 0,"postsid:\"%V\"",&sid);
//            }
//            else if((sidstrstart = ngx_strcasestrn(r->request_body->buf->start,"&sid=",4)) != NULL)
//            {
//            	sid.data = sidstrstart+5;
//
//            	if((sidstrend = ngx_strlchr(sidstrstart+1,r->request_body->buf->end,'&'))== NULL)
//            		sid.len = r->uri_end - sidstrstart - 5;
//            	else
//            		sid.len = sidstrend - sidstrstart - 5;
//
//            	ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"postsid:\"%V\"",&sid);
//            }
//            else
//            	return NGX_HTTP_BAD_REQUEST;
//            ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll3");
//        }
		...
		for(i =0; i < ctx->urisidarray->nelts; i++){
			ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll urisidarray:\"%V\"",&((ngx_str_t*)ctx->urisidarray->elts)[i]);
		...
		for(i =0; i < ctx->keyarray->nelts; i++){
			ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll tokenkeyarr:\"%V\"",&((ngx_str_t*)ctx->keyarray->elts)[i]);
...
//		sprintf((char*)hashkey.data, "%s%s", (char*)tokenkey.data, (char*)sid.data); sprinf sometimes make bug!
		...
		ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll hashkey:\"%V\"",&hashkey);
        hash = ngx_hash_key_lc(hashkey.data, hashkey.len);
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"%%zll hash(key):\"%d\"",hash);
...
//        	limit->burst *= (size_t)floor(fm/3600);
        ...
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll maxtps:\"%d\"",maxtps);
//        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,"zll limit->burst:\"%d\"",limit->burst);
...
//	ngx_array_t*         hashkeyarray;
	...
	if(!cf->cycle->dbconn)
	{
		...
		checkerr(cf->cycle->errhp,OCIServerAttach( cf->cycle->srvhp, cf->cycle->errhp, (text *)"orcl",4, 0),cf->log);
		...
		OCIAttrSet((dvoid *) cf->cycle->authp, (ub4) OCI_HTYPE_SESSION,(dvoid *) "sde", (ub4) 3,(ub4) OCI_ATTR_USERNAME, cf->cycle->errhp);
		OCIAttrSet((dvoid *) cf->cycle->authp, (ub4) OCI_HTYPE_SESSION,(dvoid *) "sde", (ub4) 3,(ub4) OCI_ATTR_PASSWORD, cf->cycle->errhp);
	...
	if(cf->cycle->dbconn)
	{
		OCIHandleAlloc( cf->cycle->envhp, &cf->cycle->stmthp,OCI_HTYPE_STMT, 0, 0);
		checkerr(cf->cycle->errhp, OCIStmtPrepare(cf->cycle->stmthp, cf->cycle->errhp,(const OraText*) ngx_http_access_control_sql.data,ngx_http_access_control_sql.len, OCI_NTV_SYNTAX, OCI_DEFAULT),cf->log);
		...
		keylimittemp->uri.data = (text *) ngx_pcalloc(cf->pool, 33);
		keylimittemp->uri.len = 33;
		keylimittemp->sid.data = (text *) ngx_pcalloc(cf->pool, 7);
		keylimittemp->sid.len = 7;

		OCIDefineByPos(cf->cycle->stmthp,&hDefine1,cf->cycle->errhp,1,keylimittemp->key.data,keylimittemp->key.len, SQLT_CHR, NULL, NULL, NULL, OCI_DEFAULT);
		OCIDefineByPos(cf->cycle->stmthp,&hDefine2,cf->cycle->errhp,2,keylimittemp->uri.data,keylimittemp->uri.len, SQLT_CHR, NULL, NULL, NULL, OCI_DEFAULT);
		OCIDefineByPos(cf->cycle->stmthp,&hDefine3,cf->cycle->errhp,3,&keylimittemp->maxtps,sizeof(keylimittemp->maxtps), SQLT_INT, NULL, NULL, NULL, OCI_DEFAULT);
		OCIDefineByPos(cf->cycle->stmthp,&hDefine4,cf->cycle->errhp,4,keylimittemp->sid.data,keylimittemp->sid.len, SQLT_CHR, NULL, NULL, NULL, OCI_DEFAULT);
		checkerr(cf->cycle->errhp,OCIStmtExecute( cf->cycle->svchp, cf->cycle->stmthp, cf->cycle->errhp, (ub4) 1, (ub4) 0,(OCISnapshot *) NULL,(OCISnapshot *) NULL, (ub4)OCI_STMT_SCROLLABLE_READONLY ),cf->log);
	//	swResult = OCIStmtExecute( cf->cycle->svchp, cf->cycle->stmthp, cf->cycle->errhp, (ub4) 1, (ub4) 0,(OCISnapshot *) NULL,(OCISnapshot *) NULL, (ub4)OCI_STMT_SCROLLABLE_READONLY );
	//
...
//		hash_init.temp_pool           = NULL;
	...
	hash_init.hash                = ctx->keyhash;                                     // hash结构
	hash_init.key                 = &ngx_hash_key_lc;                                 // hash算法函数
	hash_init.max_size            = 1024*100;                                         // max_size
	hash_init.bucket_size         = 128;                                               // ngx_align(64, ngx_cacheline_size);
	hash_init.name                = "RestRequestLimit_DataPool";                      // 在log里会用到
	...
	hash_init.temp_pool           = NULL;
		...
		ctx->urisidarray = ngx_array_create(cf->pool, 100, sizeof(ngx_str_t));
...
//			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,"key_hash:\"%d\"",arr_node->key_hash);
			...
			ngx_memzero(keylimittemp->uri.data, 33);
			ngx_memzero(keylimittemp->sid.data, 7);
			swResult=OCIStmtFetch2(cf->cycle->stmthp,cf->cycle->errhp,1,OCI_FETCH_NEXT,1,OCI_DEFAULT);
